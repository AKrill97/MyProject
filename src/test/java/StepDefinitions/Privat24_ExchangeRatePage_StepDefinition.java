...
public class Privat24_ExchangeRatePage_StepDefinition {
    ...
    @When("^Get the '(.*)' exchange rate on page and save them in local storage$")
    public void get_the_currencies_exchange_rate(String currency) {
        exchangeRatePage.isExchangeRatePageOpened()
                .getExchangeRatesByCurrencyAndSaveInLocalStorage(currency);
    }
}
